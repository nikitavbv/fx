...
impl From<fx_api::capnp::struct_list::Reader<'_, fx_api::fx_capnp::sql_result_row::Owned>> for SqlResult {
    fn from(value: fx_api::capnp::struct_list::Reader<'_, fx_api::fx_capnp::sql_result_row::Owned>) -> Self {
        let rows = value.into_iter()
            .map(|v| SqlResultRow {
                columns: v.get_columns().unwrap()
                    .into_iter()
                    .map(|v| {
                        use fx_api::fx_capnp::sql_value::value::{Which as ProtocolValue};
