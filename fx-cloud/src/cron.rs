        ...
        spawn(move || {
            loop {
                ...
                for task in tasks_to_run {
                    if let Err(err) = cloud_engine.invoke_service::<CronRequest, ()>(cloud_engine.clone(), &ServiceId::new(task.function_id), &task.rpc_function_name, CronRequest {}) {
                        error!("failed to run cron task: {err:?}");
                    } else {
