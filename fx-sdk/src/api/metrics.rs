use {
    thiserror::Error,
    fx_types::{capnp, abi_metrics_capnp, abi::{MetricsCounterRegisterResult, MetricsCounterRegisterResultCode}},
...
impl Counter {
    pub fn new(name: impl Into<String>) -> Result<Self, with_label_values::CounterRegisterError> {
...
pub mod with_label_values {
    pub use super::*;

    #[derive(Debug, Error)]
    pub enum CounterRegisterError {
        #[error("internal sdk error")]
        InternalSdkError,
    }

    impl CounterTemplate {
        pub fn with_label_values(&self, values: Vec<String>) -> Result<Counter, CounterRegisterError> {
            ...
            let mut result = std::mem::MaybeUninit::<MetricsCounterRegisterResult>::zeroed();
