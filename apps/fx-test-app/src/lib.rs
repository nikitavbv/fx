...
async fn test_sql_contention_busy() -> &'static str {
    ...
    let result = database.exec(SqlQuery::new("with recursive cnt(x) AS (select 1 union all select x + 1 from cnt where x < 8000) insert into contention_busy (v) select randomblob(10000) from cnt")).await;
    if let Err(err) = result {
        match err {
...
async fn test_fetch(headers: axum::http::HeaderMap) -> impl IntoResponse {
    fetch(
...
async fn test_fetch_timeout() -> &'static str {
    match fetch(HttpRequest::get("http://10.255.255.1").unwrap()).await {
        ...
        Err(FetchError::FunctionNotFound)
        | Err(FetchError::FunctionPanicked)
        | Err(FetchError::FunctionBusy)
        | Err(FetchError::FunctionCrashed)
        | Err(FetchError::FunctionInstantiationError) => "rpc call error",
